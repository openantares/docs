...
Earlier releases were published under a versioned name, `ant-0.1.schema.json`; that document stays served for anything still referencing it, but the unversioned `$id` above is the identifier every current release carries — the schema for formats 0.7 and 1.0 lives there today, and a later release replaces it in place.
...
<details>
<summary>Show the full schema (4,089 lines)</summary>

```json
{
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "$id": "https://openantares.org/schema/ant.schema.json",
  "title": "OpenAntares .ant record (formats 0.7 and 1.0)",
  ...
  "oneOf": [
    ...
    {
      "$ref": "#/$defs/original_chunk"
    },
    {
      "$ref": "#/$defs/original_source"
    },
    {
      "$ref": "#/$defs/trailer"
    },
  ...
  "$defs": {
    ...
    "counts": {
      ...
      "properties": {
        ...
        "originalChunks": {
          "description": "`original_chunk` records. v1.0 only; omitted when zero, so a file without originals has exactly the v0.7 trailer.",
          "type": "integer",
          "format": "uint64",
          "minimum": 0
        },
        "originalSources": {
          "description": "`original_source` records. v1.0 only; omitted when zero.",
          "type": "integer",
          "format": "uint64",
          "minimum": 0
        },
    ...
    "derivation": {
      "description": "A cleaned-text Evidence's typed, immutable binding to the stored original it was normalized from (format v1.0, contract [`NORMALIZED_TEXT_CONTRACT`]). Carried only by a derivative: ordinary blob-free Evidence whose `content` is the cleaned text. The binding (`primary_evidence_id`, `asset_id`, `sha256`, `byte_length`) must equal the primary's [`SourceBlob`]; `text_sha256`/`text_byte_length` must equal the digest and UTF-8 length of the record's `content`. The slot `(primary_evidence_id, job_id, segment.index)` holds exactly one derivative. Once written, a derivation is never changed or dropped.",
      "type": "object",
      "required": [
        "contract",
        "primaryEvidenceId",
      ...
      "properties": {
        "assetId": {
          "description": "That primary's `sourceBlob.assetId`, `[A-Za-z0-9_-]{16,128}`.",
          "type": "string",
          "pattern": "^[A-Za-z0-9_-]{16,128}$"
        },
        "byteLength": {
          "description": "That primary's `sourceBlob.byteLength`.",
          "type": "integer",
          "format": "uint64",
          "minimum": 0
        },
        "contract": {
          "description": "Always [`NORMALIZED_TEXT_CONTRACT`].",
          "type": "string",
          "const": "antares.normalized-text/v1"
        },
        "jobId": {
          "description": "The producer's job, `[A-Za-z0-9_.:-]{1,128}`.",
          "type": "string",
          "pattern": "^[A-Za-z0-9_.:-]{1,128}$"
        },
        "normalizer": {
          "description": "The normalizer that produced this text.",
          "$ref": "#/$defs/normalizer"
        },
        "primaryEvidenceId": {
          "description": "The primary Evidence carrying the original: non-empty, with no control characters.",
          "type": "string",
          "pattern": "^[^\\u0000-\\u001F\\u007F-\\u009F]+$"
        },
        "segment": {
          "description": "This derivative's slot, locator and coverage.",
          "$ref": "#/$defs/derivation_segment"
        },
        "sha256": {
          "description": "That primary's `sourceBlob.sha256`.",
          "type": "string",
          "pattern": "^[0-9a-f]{64}$"
        },
        "textByteLength": {
          "description": "UTF-8 byte length of the record's `content`.",
          "type": "integer",
          "format": "uint64",
          "minimum": 0
        },
        "textSha256": {
          "description": "SHA-256 of the record's `content` (its UTF-8 bytes).",
          "type": "string",
          "pattern": "^[0-9a-f]{64}$"
        }
      },
    ...
    "derivation_segment": {
      "description": "Where one derivative sits in its job: a stable 0-based slot (slots may be sparse while work is pending, partial or refused), the source locator it covers, and the producer's opaque per-segment coverage facts.",
      "type": "object",
      "required": [
        "index",
        "locator",
      ...
      "properties": {
        "coverage": {
          "description": "The producer's coverage facts for this segment: a JSON object of at most [`Derivation::COVERAGE_MAX_BYTES`] serialized, nesting at most [`Derivation::COVERAGE_MAX_DEPTH`] levels, carried verbatim and never interpreted by the engine. It says nothing about the whole job's readiness.",
          "type": "object",
          "additionalProperties": true
        },
        "index": {
          "description": "Stable 0-based slot within the job. Not assumed dense, and not assumed to equal a parser-unit ordinal.",
          "type": "integer",
          "format": "uint64",
          "minimum": 0
        },
        "locator": {
          "description": "Where in the original this text came from, at most [`Derivation::LOCATOR_MAX_BYTES`] bytes (UTF-8), with no control characters. The schema bounds characters; readers bound the bytes.",
          "type": "string",
          "pattern": "^[^\\u0000-\\u001F\\u007F-\\u009F]+$",
          "maxLength": 2048,
          "minLength": 1
      ...
      "properties": {
        "data": {
          ...
          "properties": {
            ...
            "derivation": {
              "description": "The typed binding of a cleaned-text derivative to the stored original it was normalized from (format v1.0). Absent on every other record, which keeps their bytes unchanged. Never present together with `source_blob`.",
              "anyOf": [
                {
                  "$ref": "#/$defs/derivation"
                },
            ...
            "source_blob": {
              "description": "The stored original this Evidence was cut from, when it is the primary record of a file (format v1.0). Absent on every other record, which keeps their bytes unchanged. The original itself is never embedded here.",
              "anyOf": [
                {
    ...
    "normalizer": {
      "description": "Which normalizer produced a derivative: stable parser identity, its version, and the digest of its configuration. A new version or configuration is a new job, never a rewrite.",
      "type": "object",
      "required": [
        "name",
        "version",
      ...
      "properties": {
        "configurationSha256": {
          "description": "SHA-256 of the normalizer configuration, 64 lowercase hex characters.",
          "type": "string",
          "pattern": "^[0-9a-f]{64}$"
        },
        "name": {
          "description": "Stable parser identity, `[A-Za-z0-9_.:-]{1,64}`.",
          "type": "string",
          "pattern": "^[A-Za-z0-9_.:-]{1,64}$"
        },
        "version": {
          "description": "Parser version, `[A-Za-z0-9_.:-]{1,64}`.",
          "type": "string",
          "pattern": "^[A-Za-z0-9_.:-]{1,64}$"
        }
      },
    ...
    "original_chunk": {
      "description": "One chunk of an Evidence's stored original (v1.0), right after that Evidence. See [`OriginalChunk`].",
      "type": "object",
      "required": [
        "kind",
        "data"
      ...
      "properties": {
        "data": {
          "description": "One chunk of a stored original (v1.0). Every chunk of an Evidence's original follows that Evidence record immediately, in order, before any other record; together they are exactly `sourceBlob.byteLength` bytes with digest `sourceBlob.sha256`. All chunks but the last have the length of the first; the last is no longer. [`AntReader`] enforces every one of those rules as it reads, so a reader that returns without error has verified each original it passed through.",
          "type": "object",
          "required": [
            "evidenceId",
            "assetId",
          ...
          "properties": {
            "assetId": {
              "description": "That Evidence's `sourceBlob.assetId`.",
              "type": "string"
            },
            "byteOffset": {
              "description": "Offset of the chunk's first byte in the original.",
              "type": "integer",
              "format": "uint64",
              "minimum": 0
            },
            "bytes": {
              "description": "The chunk's bytes, standard base64 with padding.",
              "type": "string"
            },
            "evidenceId": {
              "description": "The Evidence record this chunk belongs to — the one just before it.",
              "type": "string"
            },
            "index": {
              "description": "Position in the original, from 0.",
              "type": "integer",
              "format": "uint64",
              "minimum": 0
            },
            "sha256": {
              "description": "SHA-256 of the decoded chunk, 64 lowercase hex.",
              "type": "string",
              "pattern": "^[0-9a-f]{64}$"
            }
          },
        ...
        "kind": {
          "type": "string",
          "const": "original_chunk"
        }
      },
      "additionalProperties": true
    },
    "original_source": {
      "description": "One source reference of an Evidence's stored original (v1.0): where the bytes came from. All of them follow that original's last chunk, in strictly increasing `referenceId` order, bound to exactly that Evidence's `sourceBlob`.",
      "type": "object",
      "required": [
        "kind",
        "data"
      ...
      "properties": {
        "data": {
          "description": "Where an Evidence's stored original came from — a native folder or picker, a drive item, a mail message (format v1.0). Any number bind to one original; they are append-only and immutable, and appending one never changes the Evidence record itself. The binding (`evidence_id`, `asset_id`, `sha256`, `byte_length`) must equal that Evidence's [`SourceBlob`]: a reference names exact bytes.",
          "type": "object",
          "required": [
            "evidenceId",
            "assetId",
          ...
          "properties": {
            ...
            "byteLength": {
              "description": "That Evidence's `sourceBlob.byteLength`.",
              "type": "integer",
              "format": "uint64",
              "minimum": 0
            },
            "evidenceId": {
              "description": "The primary Evidence whose original this describes: non-empty, with no control characters.",
              "type": "string",
              "pattern": "^[^\\u0000-\\u001F\\u007F-\\u009F]+$"
            },
            "recordedAt": {
              "description": "When the reference was first recorded.",
              "type": "string",
              "format": "date-time"
            },
            "referenceId": {
              "description": "Client-chosen, deterministic for one source observation, so a retry converges: `[A-Za-z0-9_.:-]{1,128}`.",
              "type": "string",
              "pattern": "^[A-Za-z0-9_.:-]{1,128}$"
            },
            "sha256": {
              "description": "That Evidence's `sourceBlob.sha256`.",
              "type": "string",
              "pattern": "^[0-9a-f]{64}$"
            },
            "source": {
              "description": "Where the bytes came from: a JSON object of at most [`SourceReference::SOURCE_MAX_BYTES`] serialized, nesting at most [`SourceReference::SOURCE_MAX_DEPTH`] levels, opaque to the engine.",
              "type": "object",
              "additionalProperties": true
            }
          },
    ...
    "source_blob": {
      "description": "The exact original a primary Evidence was cut from, stored outside the record (format v1.0). The record carries this small reference; the bytes live in the engine's original store and travel beside the record wherever it goes (the feed carries only the reference and each node transfers the bytes before applying it; a blob-bearing `.ant` file carries them as `original_chunk` records right after the Evidence). `asset_id` names the stored original on every node and is never an authorization capability: reading it is authorized by the Evidence's own visibility. An Evidence's original is immutable — a changed file is a new Evidence id.",
      "type": "object",
      "required": [
        "assetId",
        "byteLength",
      ...
      "properties": {
        "assetId": {
          "description": "Opaque identity of the stored original, `[A-Za-z0-9_-]{16,128}`.",
          "type": "string",
          "pattern": "^[A-Za-z0-9_-]{16,128}$"
        },
        "byteLength": {
          "description": "Exact length of the original, in bytes. Zero is an empty file.",
          "type": "integer",
          "format": "uint64",
          "minimum": 0
        },
        "fileName": {
          "description": "Display name of the original, 1..=1024 bytes with no control characters. Never used as a filesystem path. The schema bounds characters; readers bound the bytes.",
          "type": "string",
          "pattern": "^[^\\u0000-\\u001F\\u007F-\\u009F]+$",
          "maxLength": 1024,
          "minLength": 1
        ...
        "mediaType": {
          "description": "Media type declared for the original: informational, never used to serve it. 1..=255 printable ASCII bytes.",
          "type": "string",
          "pattern": "^[\\x20-\\x7E]{1,255}$"
        },
        "sha256": {
          "description": "SHA-256 of the original bytes, 64 lowercase hex characters.",
          "type": "string",
          "pattern": "^[0-9a-f]{64}$"
        }
      },
    ...
    "unknown_kind": {
      "description": "Forward compatibility: any object with a string `kind` outside the v0.7 / v1.0 vocabulary is valid at the container level and MUST be skipped by readers.",
      "type": "object",
      "required": [
        "kind"
      ],
