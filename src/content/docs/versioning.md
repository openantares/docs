...
description: Why the Rust crates are 0.3.x while the .ant format is 0.5.x — deliberately independent version lines, SUPPORTED_FORMAT_VERSION, and the same-major compatibility rule enforced at read time.
---

**The crate version and the format version are deliberately not the same number.** The published crates are at `0.3.0`; the format they implement is `0.5`. This page exists so that difference reads as the design decision it is, not a mistake.
...
|----------|---------|-------|
| `.ant` format | **0.5** | [specification](../format/spec/) · [v0.5.0 release](https://github.com/openantares/ant/releases/tag/v0.5.0) |
| `ant-types` | 0.3.0 | [crates.io](https://crates.io/crates/ant-types) · [docs.rs](https://docs.rs/ant-types) |
| `antares-format` | 0.3.0 | [crates.io](https://crates.io/crates/antares-format) · [docs.rs](https://docs.rs/antares-format) |
| `openantares` (CLI) | 0.3.0 | [crates.io](https://crates.io/crates/openantares) · [docs.rs](https://docs.rs/openantares) |
...
```text
format version:  0.5 (this build reads/writes 0.5.x)
```
